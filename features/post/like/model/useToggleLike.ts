...
export function useToggleLike(postId: number) {
  ...
  return useMutation({
    ...
    const previousPost = queryClient.getQueryData<PostDetailView>(queryKeys.post(postId));

    // 좋아요 토글
    queryClient.setQueryData(queryKeys.like(postId), !previousLike);

    // 좋아요 수 업데이트
    queryClient.setQueryData(queryKeys.post(postId), (old: PostDetailView | undefined) => {
      if (!old) return old;
      return {
        ...old,
        likesCount: previousLike ? old.likesCount - 1 : old.likesCount + 1
      };
    });
