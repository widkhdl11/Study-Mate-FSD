...
export default async function HeroStats() {
    ...
    return (
        <div className="grid grid-cols-3 gap-6 border-t-2 border-dashed border-paper-line pt-6">
            {stats.map((s) => (
                <div key={s.label} className="text-center lg:text-left space-y-1">
                    <div className="text-3xl md:text-4xl font-bold text-ink tabular-nums">
                        {s.value}
                    </div>
        ...
        <div className="grid grid-cols-3 gap-6 border-t-2 border-dashed border-paper-line pt-6">
            {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="text-center lg:text-left space-y-1">
                    <div className="h-9 md:h-10 w-20 bg-paper-line rounded-md animate-pulse mx-auto lg:mx-0" />
                    <div className="h-4 w-16 bg-paper-line rounded-md animate-pulse mx-auto lg:mx-0" />
                </div>
            ))}
        </div>
    )
